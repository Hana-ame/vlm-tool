...
(function () {
  ...
  const $style = (stylesheet) =>
    $el("style", stylesheet, (el) => document.head.appendChild(el));
  ...
  const uhpConfig = (() => {
    const _conf = Object.assign(
      { abg: true, mt: true, pe: true, fw: false },
      GM_getValue("uhp")
    );
  ...
  if (uhpConfig.mt) {
    (async () => {
      ...
      if (u.searchParams.get("report") === "select") {
        return;
      }
      if (u.searchParams.get("act") === "expunge") {
        return;
      }
      ...
      if (!imgParentEl) {
        return console.error("No imgParentEl");
      }
      ...
      async function* newPagedImgElsGen(initUrl) {
        ...
        while (url) {
          ...
          if (resp.ok) {
            ...
            imgEls = Array.from(
              $find(docEl, IMAGE_PARENT_SELECTOR)?.children ?? []
            );
  ...
  if (uhpConfig.pe) {
    (async () => {
      ...
      const NEXT_PAGE_SELECTOR =
        '.ptt td:last-child > a, .searchnav a[href*="next="]';
      const IMAGE_PARENT_SELECTOR = isTableLayout
        ? "table.itg > tbody"
        : "div.itg";
      ...
      $("table.ptb, .itg + .searchnav, #favform + .searchnav").replaceWith(
        statusEl
      );
  ...
  uhpDialogEl.className = location.host === window.location.host ? "dark" : "";
